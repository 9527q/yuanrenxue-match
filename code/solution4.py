import requests
import re
import json

re_src_class_style = re.compile(
    r'.*src="(.*?)" class="(.*?)" style="left:(.*?)px"'
)

# 两层列表，一个内层列表组合成一个整数，一个整数由多个数字组成，一个数字：(img_str, src, class, location)
num_list = []
all_type_img = {}  # src -> img_str
for page_no in range(1, 6):
    p = requests.get(f'http://match.yuanrenxue.com/api/match/4?page={page_no}')
    data = p.json()
    html = data['info']
    num_list_now = []
    # 存储真正要显示的标签的类
    right_class_set = set()
    for td_str in re.findall(r'<td>.*?</td>', html):
        num_info = []
        class_set = set()
        for img_str in re.findall(r'<img .*?>', td_str):
            match = re_src_class_style.match(img_str)
            src = match.group(1)
            class_ = match.group(2)
            style = int(match.group(3))
            num_info.append((img_str, src, class_, style//11))
            class_set.add(class_)
            # 记录数字数据（一共有 10 种）
            if src not in all_type_img:
                all_type_img[src] = img_str
        num_list_now.append(num_info)
        # 如果这组数据中只有一种class，那说明就是这次请求真正的class
        if len(class_set) == 1:
            right_class_set |= class_set
    # 对 class 进行再次验证，一次请求中的正确 class 是唯一的
    assert len(right_class_set) == 1
    right_class = right_class_set.pop()
    # 将不显示的数字清除掉
    num_list.extend(
        [i for i in num_info if i[2] == right_class]
        for num_info in num_list_now
    )
    print(page_no)



# 1. 取消下面的注释，运行代码
# 2. 浏览器打开 s4.html,将数字结果按顺序填入 s4.json 中的键的位置
# 3. 给下面代码打上注释，运行代码，得到题解

# assert len(all_type_img) == 10
# sl1 = []
# sl2 = []
# for src, img_str in all_type_img.items():
#     sl1.append(img_str)
#     sl2.append(f'"": "{src}",')
# sl2[-1] = sl2[-1][:-1]
# with open('s4.html', 'w') as f:
#     f.write('\n'.join(sl1))
# with open('s4.json', 'w') as f:
#     f.write('{\n\t' + '\n\t'.join(sl2) + '\n}')
# exit()

# ==================================================================
# ==================================================================
#
# with open('s4.json', 'r', encoding='utf8') as f:
#     numstr2src = json.load(f)  # type: dict
numstr2src = {
	"6": "data:image/png;base64,",
	"2": "data:image/png;base64,",
	"4": "data:image/png;base64,",
	"8": "data:image/png;base64,",
	"0": "data:image/png;base64,",
	"1": "data:image/png;base64,",
	"9": "data:image/png;base64,",
	"3": "data:image/png;base64,",
	"7": "data:image/png;base64,",
	"5": "data:image/png;base64,"
}
src2numstr = {src: num for num, src in numstr2src.items()}

number_sum = 0
for num_info in num_list:
    s = [' '] * 4
    location = 0
    for i in num_info:
        s[location+i[3]] = src2numstr[i[1]]
        location += 1
    number_sum += int(''.join(s))

print(f'所有数的和：{number_sum}')
